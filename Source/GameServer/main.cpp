...
int main()
{
	...
	if (netConfig.Load("..\\Config\\GameServer.xml") == false)
	{
		wprintf_s(L"Failure load GameServer.xml");
		return 0;
	}

	if (netConfig.Verify() == false)
	{
		wprintf_s(L"Failure verify GameServer.xml");
		return 0;
	}

	auto pServer = std::make_unique<GameServer>(netConfig);
