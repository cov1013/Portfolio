...
MonitorThread::MonitorThread(NetDevice* pServer, ActorManager* pActorManager)
{
...
void MonitorThread::Run()
{
	...
	_thread = std::thread([this]()
	{
		CPUMeter cpuMeter;
#ifdef _DEBUG
		PerformanceDataHelper pdh(L"ChatServer_x64_Debug");
#else
		PerformanceDataHelper pdh(L"ChatServer_x64_Release");
#endif
		...
		while (_bActive == true)
		{
			...
			LONGLONG commitMemSize = pdh.GetProcessCommitMemory();
			if (commitMemSize > CommitLimitSize || commitMemSize < 0)
			{
				LOG(eLogLevel::System, L"Leak of Commit Memory (%d bytes)\n", commitMemSize);
				CrashDumper::Crash();
			}
